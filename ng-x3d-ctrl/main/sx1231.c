...
bool is_dio_mode_match(sx1231_mode_t mode, sx1231_dio_mode_t dio_mode)
{
    ...
    case SX1231_DIO_MODE_RX:
        if (mode == SX1231_MODE_RECEIVER)
        {
            return true;
        }
        break;
    case SX1231_DIO_MODE_TX:
        if (mode == SX1231_MODE_TRANSMITTER)
        {
            return true;
        }
        break;
    case SX1231_DIO_MODE_BOTH:
        if (mode == SX1231_MODE_TRANSMITTER || mode == SX1231_MODE_RECEIVER)
        {
            return true;
        }
        break;
...
esp_err_t sx1231_init(spi_device_handle_t spi, sx1231_context_t **out_ctx)
{
    ...
    *ctx = (sx1231_context_t){
    ...
    start = millis();
    while (((readReg(ctx->spi, SX1231_REG_IRQ_FLAGS_1) & SX1231_IRQ1_MODE_READY) == 0) && millis() - start < timeout)
        ; // wait for ModeReady
