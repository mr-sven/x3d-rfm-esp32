...
esp_err_t update_dio(sx1231_context_t* ctx)
{
    ...
    for (int i = 0; i < SX1231_DIO_MAPPING_COUNT; i++)
    {
        ...
        {
            data |= ctx->dio[i].dio_type << ctx->dio[i].pin;
        }
    }
...
esp_err_t sx1231_mode(sx1231_context_t* ctx, sx1231_mode_t mode)
{
    ...
    ctx->mode = mode;
    writeReg(ctx->spi, SX1231_REG_OP_MODE, mode | (ctx->sequencerOff << 7));
...
esp_err_t sx1231_dio_mapping(sx1231_context_t* ctx, sx1231_dio_pin_t pin, sx1231_dio_type_t type, sx1231_dio_mode_t mode)
{
    ...
    ctx->dio[dio_pin_to_index(pin)].dio_type = type;
    ctx->dio[dio_pin_to_index(pin)].dio_mode = mode;
    return update_dio(ctx);
}
...
esp_err_t sx1231_transmit(sx1231_context_t* ctx, uint8_t *buffer, size_t size)
{
    ...
    sx1231_mode(ctx, SX1231_MODE_TRANSMITTER);

    start = millis();
    while (((readReg(ctx->spi, SX1231_REG_IRQ_FLAGS_2) & SX1231_IRQ2_PACKET_SENT) == 0) && millis() - start < timeout); // wait for packet send
    return sx1231_mode(ctx, SX1231_MODE_STANDBY);
}